function p_d = p_d_y2_ideal(snr_dB_rng, p_fa, h, N_seq, fOff, N_frames, N_rep)
...
for iSnr = 1:N_snr
  ...
  freq_off_factor .* repmat(h, 1, N_frames); ...
                ...
                N_h0, 'absSq');
        p_d_mid_val = p_d_mid_val + sum(y_(idx_mid,:) > r_opt_coeff * sigmaw2_curr,2);
        p_d_all_vals = p_d_all_vals + sum(any(y_(idx_rng,:) > r_opt_coeff * sigmaw2_curr,1),2);
    end
