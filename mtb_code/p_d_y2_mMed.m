function p_d = p_d_y2_mMed(snr_dB_rng, p_fa, h, N_seq, fOff, N_frames, N_rep, L_av)
...
[r_opt, p_fa_final, iIter] = set_thr_forGiven_pFa( ...
        h, N_seq, p_fa, 'absSq', 'mMed', 1e6, L_av); 
%(h, N_h0, p_fa_req, abs_absSq, mAv_mMed, N_samps, L_av)
...
for iSnr = 1:N_snr
    ...
    parfor iRep = 1:N_rep
        y_ = MF_correlate(h, ...
                    [zeros(N_h + L_av,N_frames); ...
  freq_off_factor .* repmat(h, 1, N_frames); ...
                    zeros(N_h,N_frames)] + ...
                    sqrt(sigmaw2_curr/2) * ...
                        (randn(3*N_h + L_av,N_frames) + 1i*randn(3*N_h + L_av,N_frames)),...
                N_h0, 'absSq');
        z = movmedian([zeros(L_av-1,N_frames); y_],L_av,'Endpoints','discard');
        p_d_mid_val = p_d_mid_val + sum(y_(idx_mid,:) > r_opt * z(idx_mid-1, :),2);
        p_d_all_vals = p_d_all_vals + sum(any(y_(idx_rng,:) > r_opt * z(idx_rng-1, :),1),2);
    end
