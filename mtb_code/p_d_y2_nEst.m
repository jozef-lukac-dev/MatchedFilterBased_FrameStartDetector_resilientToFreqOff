...
for iSnr = 1:N_snr
  ...
  freq_off_factor .* repmat(h, 1, N_frames); ...
        ...
        p_d_mid_val = p_d_mid_val + sum(y_(idx_mid,:) > r_opt_coeff * z_nVar_est(idx_mid2, :),2);
        p_d_all_vals = p_d_all_vals + sum(any(y_(idx_rng,:) > r_opt_coeff * z_nVar_est(idx_rng2, :),1),2);
    end
